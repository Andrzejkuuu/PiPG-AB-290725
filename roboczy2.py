...
X_end = list[list_of_end_pointID[index]][1]
Y_end = list[list_of_end_pointID[index]][2]

pointID_begin = list[list_of_begin_pointID[index]][0]
X_begin = list[list_of_begin_pointID[index]][1]
Y_begin = list[list_of_begin_pointID[index]][2]
...
for point in list:
    ...
    if pointID_center == min(array(list)[:, 0]):
        ...
        if X_center == X_end and Y_center == Y_end:
            print pointID_begin
            print pointID_end
            ...
            angle_in = angleIn(list[pointID_center - 1][1], list[pointID_center - 1][2], X_center, Y_center, list[pointID_begin + 1][1],  list[pointID_begin + 1][2])
            print('%s - %s - %s: %0.8f [deg]' % (pointID_center - 1, pointID_center, pointID_begin + 1, angle_in))
            print('-------------------------------------------')
        ...
        elif X_center == X_begin and Y_center == Y_begin:
            print pointID_begin
            print pointID_end
            length_in = segmentLength(list[pointID_end - 1][1], list[pointID_end - 1][2], X_center, Y_center)
            print('%s - %s : %0.3f [m]' % (pointID_end - 1, pointID_center, length_in))
            length_out = segmentLength(X_center, Y_center, list[pointID_begin + 1][1], list[pointID_begin + 1][2])
            print('%s - %s : %0.3f [m]' % (pointID_center, pointID_center + 1, length_out))
            angle_in = angleIn(list[pointID_end - 1][1], list[pointID_end - 1][2], X_center, Y_center, list[pointID_center + 1][1],  list[pointID_center + 1][2])
            print('%s - %s - %s: %0.8f [deg]' % (pointID_end - 1, pointID_center, pointID_center + 1, angle_in))
            print('-------------------------------------------')
            index += 1
